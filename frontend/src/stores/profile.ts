...
export const useProfileStore = defineStore('profile', () => {
  ...
  const profile = ref<Profile>({
    ...
    occupation: '学生',
    siteTitle: '霜雪旧曾谙的世界',
    siteSubtitle: '这里记录着我的技术分享、生活感悟和对世界的思考',
  ...
  const loadProfile = async (forceRefresh = false) => {
    try {
      ...
      profile.value = {
        id: data.id ?? 1,
        name: data.name || '霜雪旧曾谙',
        avatar: data.avatar || '/avatar.jpg',
        bio: data.bio || '计算机专业学生 | 二次元爱好者 | 海洋探索者 | 哲学思考者',
        email: data.email || 'example@email.com',
        location: data.location || '中国',
        website: data.website || 'https://example.com',
        socialLinks: links.length > 0 ? links : [
          { platform: 'github', url: 'https://github.com/username', displayName: 'GitHub', icon: '🐙', isPublic: true },
          { platform: 'twitter', url: 'https://twitter.com/username', displayName: 'Twitter', icon: '🐦', isPublic: true },
          { platform: 'website', url: 'https://example.com', displayName: 'Website', icon: '🌐', isPublic: true },
          { platform: 'email', url: 'mailto:example@email.com', displayName: '邮箱', icon: '📧', isPublic: true }
        ],
        skills: data.skills && data.skills.length > 0 ? data.skills : ['Vue.js', 'Python', 'Flask', 'MySQL', 'TypeScript', 'Tailwind CSS'],
        interests: data.interests && data.interests.length > 0 ? data.interests : ['二次元', '海洋', '自然', '哲学', '技术分享'],
        education: data.education || '计算机科学与技术',
        occupation: data.occupation || '学生',
        siteTitle: data.site_title || '霜雪旧曾谙的世界',
        siteSubtitle: data.site_subtitle || '这里记录着我的技术分享、生活感悟和对世界的思考',
  ...
  const saveProfile = async () => {
    try {
      ...
      const payload = {
        ...
        site_title: profile.value.siteTitle || '',
        site_subtitle: profile.value.siteSubtitle || '',
